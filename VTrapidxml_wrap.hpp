// Wrapper for rapidxml C++ library
...
namespace VT
{
	...
	template<typename T> struct type { };
         ...
         * the value to this type and throws VT::xml_general_error if conversino failes.
         * Otherwise, uses std::operator>>() to read and return the value
         * of type T.
         * Throws VT::xml_general_error if XML document is not initialized or value
         * can't be converted and VT::xml_key_error if key is empty or does not exist.
         */
        template <typename T>
         ...
         * Throws: same as usual get() function + VT::xml_general_error if dest_size is not
        ...
        bool get(const string_t& key, Ch* dest, int dest_size)
        {
            try
            {
                ...
                dest[pair.second] = VT::ensure_tchar<Ch>('\0');
         ...
         * Throws VT::xml_general_error if XML document is not initialized and
         * VT::xml_key_error if key is empty.
         */
        template <typename T>
    ...
    private:
        ...
        public:
            ...
            {
                VT::split(key, &path_, VT::ensure_tchar<Ch>("."));
                if (path_.size() == 0)
                    last_is_attribute_ = false;
                else
                    last_is_attribute_ = (path_.back().front() == VT::ensure_tchar<Ch>('<') &&
                                          path_.back().back() == VT::ensure_tchar<Ch>('>') );
            }
        ...
        std::pair<Ch*, size_t> find_value(const Path& key)
        {
            ...
            if (key.is_attribute())
            {
                ...
                if (!attr)
                    throw xml_key_error("No such attribute '" + VT::ensure_tchar<char>(attr_name.c_str()) + "'");
        ...
        xml_node* find_node(const Path& key)
        {
            ...
            for (size_t i = 0; i < key.size() - (key.is_attribute() ? 1 : 0); ++i)
            {
                ...
                if (!node)
                    throw xml_key_error("No such node '" + VT::ensure_tchar<char>(key[i].c_str()) + "'");
            }
    ...
    public:
        ...
        explicit XmlDoc(bool use_except = true)
            ...
            , noex_(!use_except)
            , doc_(nullptr, VT::noop_delete<xml_document>())
            , buffer_()
        {}

        /*
         * Read XML document from file.
         * Throws std::runtime_error if couldn't read the file or VT::xml_general_error if
        ...
        {
            doc_ = xml_ptr(new xml_document, VT::default_delete<xml_document>());
        ...
        {
            if (own)
                doc_ = xml_ptr(doc, VT::default_delete<xml_document>());
            else
                doc_ = xml_ptr(doc, VT::noop_delete<xml_document>());
         ...
         * Throws VT::xml_general_error if XML document is not initialized.
         */
        ...
        void write_document(const std::wstring& filename)
        {
            ...
            std::ofstream outfile(VT_TO_FILENAME_ENCODING(filename));
            std::string utf8_string = VT::ensure_tchar<char>(output.str().c_str());
    ...
    inline void XmlDoc<char>::read_from_file(const std::wstring& filename)
    {
        try
        {
            ...
            if (!VT::read_file(filename, buffer_))
                throw std::runtime_error("Failed to read file: " + VT::ensure_tchar<char>(filename.c_str()));
        ...
        {
            throw xml_general_error("Error parsing file '" + VT::ensure_tchar<char>(filename.c_str()) + "': " + ex.what());
        }
    }
    ...
    inline void XmlDoc<wchar_t>::read_from_file(const std::wstring& filename)
    {
        try
        {
            ...
            if (!VT::read_file(filename, utf8_content))
                throw std::runtime_error("Failed to read file: " + VT::ensure_tchar<char>(filename.c_str()));

            buffer_ = VT::UTF8ToWstring(utf8_content);
            parse_buffer(&buffer_[0]);
